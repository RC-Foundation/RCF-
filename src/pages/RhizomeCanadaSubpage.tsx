...
const RhizomeCanadaSubpage: React.FC = () => {
  ...
  const programs = Array.from({ length: 3 }, (_, i) => ({
    ...
    gradient: [
      'var(--rc-gradient-nature)',
      'var(--rc-gradient-canadian)',
      'var(--rc-gradient-forest)',
    ][i],
  }));
  ...
  const products = [
    {
      ...
      description:
        'Ethically sourced coffee blend supporting Syrian farmers and Canadian roasters.',
      descriptionAr:
        'مزيج قهوة مصدر أخلاقياً يدعم المزارعين السوريين ومحمصي القهوة الكنديين.',
      price: '$24.99',
      image:
  ...
  return (
    <div className="rhizome-canada min-h-screen bg-gradient-to-br from-teal-50 via-sky-50 to-indigo-50">
      {/* Hero Section with Canadian Logo Integration */}
      <section className="rc-hero relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-teal-800 via-sky-800 to-indigo-900 opacity-90" />
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          >
            ...
            <p
              className={`rc-body-large text-white/90 max-w-4xl mx-auto mb-8 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
            >
              {t(
      ...
      <section className="rc-section">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2
              className={`rc-heading-2 mb-6 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
            >
              {t('our-programs', 'Our Programs', 'برامجنا')}
            </h2>
            <p
              className={`rc-body-large max-w-3xl mx-auto ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
            >
              {t(
            ...
            {programs.map((program, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3
                    className={`rc-heading-3 mb-4 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                  >
                    {t(
                      `program-${index}-title`,
                      program.title,
                  ...
                  <p
                    className={`rc-body mb-6 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                  >
                    {t(
                      `program-${index}-desc`,
                      program.description,
            ...
            {products.map((product, index) => (
              ...
              >
                ...
                <div className="rc-product-content">
                  <h3
                    className={`rc-product-title ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                  >
                    {t(
                      `product-${index}-title`,
                      product.title,
                  ...
                  <p
                    className={`rc-product-description ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                  >
                    {t(
                      `product-${index}-desc`,
                      product.description,
            ...
            {achievements.map((achievement, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`rc-caption ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                  >
                    {t(
                      `achievement-${index}`,
                      achievement.label,
      ...
      <section className="rc-section-alt">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <h2
                className={`rc-heading-2 mb-6 rc-maple-accent ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
              >
                {t('community-garden', 'Community Garden', 'الحديقة المجتمعية')}
              </h2>

              <p
                className={`rc-body-large mb-6 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
              >
                {t(
                  'garden-description',
              ...
              <div className="space-y-4">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`rc-body ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
                    >
      ...
      <section className="rc-hero">
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          >
            <h2
              className={`rc-heading-2 text-white mb-6 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
            >
              {t('join-community', 'Join Our Community', 'انضم إلى مجتمعنا')}
            </h2>

            <p
              className={`rc-body-large text-white/90 max-w-3xl mx-auto mb-8 ${currentLanguage.code === 'ar' ? 'font-arabic' : ''}`}
            >
              {t(
                'join-description',
